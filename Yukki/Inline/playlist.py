    ...
    buttons = [
        [
            InlineKeyboardButton(
                text=f"𝖦𝗋𝗎𝖻 𝖯𝗅𝖺𝗒𝖫𝗂𝗌𝗍",
                callback_data=f"playlist_check {user_id}|Group|{videoid}",
            ),
            InlineKeyboardButton(
                text=f"{user_name[:8]} 𝖯𝗅𝖺𝗒𝖫𝗂𝗌𝗍",
                callback_data=f"playlist_check {user_id}|Personal|{videoid}",
            ),
        ],
        [InlineKeyboardButton(text="🗯️ ᴍᴇɴᴜʏᴜ ᴋᴀᴘᴀᴛ", callback_data="close")],
    ]
    return buttons
...
def playlist_markup(user_name, user_id, videoid):
    buttons = [
        [
            InlineKeyboardButton(
                text=f"ɢʀᴜʙ ᴘʟᴀʏʟɪsᴛ",
                callback_data=f"show_genre {user_id}|Group|{videoid}",
            ),
            InlineKeyboardButton(
                text=f"{user_name[:8]} ᴘʟᴀʏʟɪsᴛɪ",
...
def add_genre_markup(user_id, type, videoid):
    buttons = [
        ...
        [
            InlineKeyboardButton(
                text="🛠️ ɢᴇʀɪ ɢɪᴛ", callback_data=f"goback {videoid}|{user_id}"
            ),
            InlineKeyboardButton(text="🗯️ ᴍᴇɴᴜʏᴜ ᴋᴀᴘᴀᴛ", callback_data="close"),
        ],
    ]
...
def third_playlist_markup(user_name, user_id, third_name, userid, videoid):
    buttons = [
        ...
        [
            InlineKeyboardButton(
                text=f"{third_name[:16]} ᴘʟᴀʏʟɪsᴛɪ",
                callback_data=f"show_genre {userid}|third|{videoid}",
            ),
        ],
        [InlineKeyboardButton(text="🗯️ ᴋᴀᴘᴀᴛ", callback_data="close")],
    ]
    return buttons
